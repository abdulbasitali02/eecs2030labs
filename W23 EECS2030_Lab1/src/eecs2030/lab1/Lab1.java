...
public class Lab1 {
	...
	public static double avg(int a, int b, int c) {
		...
		double doublesum = Double.valueOf(0.0 + a + b + c);
		double avg = doublesum / 3.0;
		return avg;
	}
	...
	public static double pressureAltitude(double pressure) {
		double altitude = 145366.45;
		double b = 1013.25;
		double c = 0.190284;

		double X = altitude * (1 - Math.pow(pressure / b,c));
	...
	public static boolean enrolStudent(int studentNumber) {
		int length = String.valueOf(studentNumber).length();
	...
	public static char middleChar(String s) {
		...
		if (length % 2 ==0 ){
			int evenMiddle = ((length/2) - 1);
			return s.charAt(evenMiddle);
		}
		else{
